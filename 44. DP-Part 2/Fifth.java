...
public class Fifth {  //O(nW)
    public static int Unboundedkanpsack(int val[],int wt[],int W){
        ...
        for(int i=1;i<n+1;i++){
            for(int j=1;j<W+1;j++){
                if(wt[i-1]<=j){ //valid
                    dp[i][j]=Math.max(val[i-1]+dp[i][j-wt[i-1]],dp[i-1][j]);  //in 0-1 after including we were finding values from (i-1) but here we'll find value from i.
                }else{
                    dp[i][j]=dp[i-1][j];
                }
            }
